...
function getData(){
    ...
    dataJseon.onload = () => {
        ...
        for(let i = 0; i < data.length; i++){
            ...
            targetList.innerHTML += `<tr><td data-index=${data[i].id}>${j}</td><td class="target-name${i}"><input type="text" readonly value="${data[i].name}"></td><td><input type="text" readonly value="${data[i].age}"></td><td><input type="text" readonly value="${data[i].job}"></td><td><div class="button-box"><span><input type="text" readonly value="${data[i].email}"></span><div><button class="correction-data" type="button" onclick="patchData(${this});">수정</button><button class="update-data" type="button">확인</button><button onclick="findName(${i});" type="button" class="del-btn">삭제</button></div></div></td></tr>`;
            j++;
        }
...
function postData(event){
    if(event.target.getAttribute('class') === 'submit-btn'){
        ...
        if (! emailCheck(email) ) {
            ...
            dataJseon.send(JSON.stringify({id : Number(targetCount.value), name : tragetName.value, age : tragetAge.value, job : targetJob.value, email : tragetEmail.value}));
            dataJseon.onload = () => {
...
function patchData(id){
    // const patchBtn = document.getElementsByClassName('correction-data');
}
